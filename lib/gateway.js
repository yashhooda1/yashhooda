...
export async function gate(req, res, {
  ...
  enforceOrigin   = true,
  plan            = null,       // e.g. 'premium' → free accounts get 403 premium_required
} = {}) {
  ...
  if (auth === 'user' && !isAdmin) {
    ...
    if (plan && user.plan !== plan) {
      res.status(403).json({ error: 'premium_required', message: 'This feature is available on the Pro plan.' });
      return { ok: false };
    }
  }
