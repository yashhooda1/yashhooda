    ...
    build: (userQuery, context = '', domain = 'running') => {
      const EXAMPLES = {
        ...
        career: [
          ...
          { q: 'Do I need a masters degree for AI Engineering?', a: 'No. Certifications (IBM AI Engineering, DeepLearning.AI) + real deployed projects beat a degree in 2026. Build in public on GitHub.' },
        ],
        travel: [
          { q: 'Best running routes in Tokyo?', a: 'Imperial Palace loop (5K, flat, iconic), Yoyogi Park trails (varied terrain), and Odaiba waterfront (bay views, 8K). Go early morning to beat heat and crowds.' },
          { q: 'How do I adjust for altitude at a race?', a: 'Arrive 3+ days early or just 1 day before. Add 3-5% to your pace targets above 5,000 ft. Hydrate aggressively and prioritize sleep the night before.' },
        ],
        general: [
...
${context ? `\nCONTEXT:\n${context}` : ''}`,
  ...
  'travel-agent': {
    label:       'Travel Agent',
    emoji:       '✈️',
    description: 'Specialized travel advisor persona with running route, altitude, and logistics expertise.',
    color:       '#06b6d4',
    build: (userQuery, context = '') => ({
      system: `You are an expert travel advisor specializing in athletic travel — marathons, trail races, and destination running. You know altitude acclimation, best running routes in major cities, race logistics, and how to plan trips around training schedules.

Your advice always covers:
- Running-specific logistics (gear, routes, race expo timing, packet pickup)
- Altitude and climate adjustments for performance
- Practical travel tips (flights, accommodation near start lines, local transport)
- What to do when not running (food, culture, sightseeing that doesn't wreck your legs)

You are direct, practical, and specific — never generic tourist recommendations.${context ? `\n\nCONTEXT:\n${context}` : ''}`,
      user: userQuery,
    }),
  },

  // ── 7. EXTENDED THINKING (Budget-aware) ──────────────────────────────────
  'extended-thinking': {
  ...
  // ── 8. PROMPT CACHING (Anthropic cache_control) ───────────────────────────
  'cached': {
