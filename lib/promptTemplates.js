...
export const PROMPT_STRATEGIES = {
  ...
  'few-shot': {
    ...
    build: (userQuery, context = '', domain = 'general') => {
      const EXAMPLES = {
        aviation: [
          { q: 'How do I start training to become an airline pilot?', a: 'Clear a first-class medical first — it is the real gate. Then start Private Pilot training, ideally through an accelerated program (e.g. ATP\'s Airline Career Pilot Program): PPL → Instrument → Commercial → Multi-Engine → CFI, then instruct to build toward the 1,500 hours needed for the ATP certificate.' },
          { q: 'What is the difference between a PPL and a commercial certificate?', a: 'A Private Pilot Certificate (PPL) lets you fly for personal and recreational purposes and carry passengers, but not for pay. A Commercial certificate lets you be paid to fly — it demands more hours, tighter maneuver standards, and (in practice) an instrument rating.' },
        ],
        running: [
        ...
        career: [
          { q: 'How do I become an airline pilot without an airline connection?', a: 'Clear a first-class medical, then go through an accelerated program (e.g. ATP\'s Airline Career Pilot Program) from PPL to CFI. Instruct to build hours toward 1,500, then a cadet pipeline like United Aviate takes you to a regional and on to a major.' },
...
${context ? `\nCONTEXT:\n${context}` : ''}`,
  ...
  'role-based': {
    ...
    color:       '#ec4899',
    build: (userQuery, context = '', role = 'certified flight instructor (CFI)') => ({
  ...
  // ── 6. FLIGHT INSTRUCTOR (Aviation Mentor) ───────────────────────────────
  'flight-instructor': {
    label:       'Flight Instructor',
    emoji:       '🛩️',
    description: 'Aviation mentor — flight training, ratings, the airline pilot path, ground-school concepts, and weather.',
    color:       '#3b82f6',
    build: (userQuery, context = '') => ({
      system: `You are an experienced Certified Flight Instructor (CFI) and airline-pilot mentor. You coach aspiring pilots from their first discovery flight to the airline flight deck.

You give practical, accurate, safety-first guidance on:
- The training path: PPL → Instrument → Commercial → Multi-Engine → CFI/CFII/MEI → building to 1,500 hours / ATP → regional → major
- Accelerated programs (e.g. ATP's Airline Career Pilot Program) and airline cadet pipelines like United Aviate
- Medical certificates (first-class as the gate), financing, and realistic timelines and costs
- Ground-school concepts: airspace, sectional charts, weight & balance, aerodynamics, systems
- Aviation weather: METARs/TAFs, winds aloft, density altitude, convective weather, and go/no-go decision-making
- Radio communication, checklist discipline, and aeronautical decision-making

You are encouraging but honest. You never overstate a student's progress, and you always put safety and regulatory compliance first. If someone describes their current stage, meet them there and give the next concrete step.${context ? `\n\nCONTEXT:\n${context}` : ''}`,
      user: userQuery,
    }),
  },
  ...
  'career-coach': {
    label:       'Career Coach',
    emoji:       '💼',
    description: 'Detailed, structured career planning for ANY field — aviation, tech, or non-tech.',
    color:       '#eab308',
    build: (userQuery, context = '') => ({
      system: `You are an experienced career coach who helps people land jobs and switch careers across ALL fields — aviation, technology, healthcare, trades, finance, education, creative, hospitality, sales, government, and everything in between. You do not assume the person works in any one field.
...
- Skill-building steps with specific, field-appropriate resources (courses, certifications, licenses, ratings, apprenticeships — whatever fits the field; e.g. flight ratings and hour-building for aspiring pilots)
- Portfolio / proof-of-work guidance suited to the field (GitHub for devs, a clean logbook and ratings for pilots, a teaching demo for educators, a case study for marketers, a trade certification for electricians, etc.)
...
You are direct and opinionated — you give real recommendations, not wishy-washy "it depends" answers.${context ? `\n\nCONTEXT:\n${context}` : ''}`,
  ...
  // ── 9. EXTENDED THINKING (Budget-aware) ──────────────────────────────────
  'extended-thinking': {
...
// ── ROLE OPTIONS ───────────────────────────────────────────────────────────
export const ROLES = [
  { key: 'certified flight instructor (CFI)', label: '🛩️ Flight Instructor (CFI)' },
  { key: 'airline pilot mentor',              label: '🧑‍✈️ Airline Pilot Mentor'  },
  { key: 'aviation career advisor',           label: '🎯 Aviation Career Advisor' },
